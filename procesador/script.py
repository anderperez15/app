...
def procesar_video(name, id):
    try:
        ...
        LOGO = "./procesador/recursos/logo.jpg"
        AUDIO = "./procesador/recursos/audio.mp3"
        EFECT = "./procesador/recursos/efect.mp4"

        print(FOLDER_FINAL+name)
        vid_title = id
        ...
        final.subclip(0,TIME).write_videofile('./procesador/procesar/'+vid_title+"ready.mp4",fps=VIDEO_FPS, codec=VIDEO_CODEC, verbose=False, progress_bar=True)

        cmd = [
            "ffmpeg",
            "-i",
            "./procesador/procesar/"+vid_title+"ready.mp4",
            "-i",
            EFECT,
            "-filter_complex",
            ...
            "{title}.mp4".format(title='./procesador/procesar/'+vid_title),
            "-y"
        ]
        #subprocess.run(cmd)
        subprocess.call(cmd)
        shutil.move('./procesador/procesar/'+vid_title+".mp4", FOLDER_FINAL)
        os.remove('./procesador/procesar/'+vid_title+"ready.mp4")
